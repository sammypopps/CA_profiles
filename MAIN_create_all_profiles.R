...
# cluster nodes
clusterExport(cl, varlist = c("raw_data", "content_length"))
...
# source("functions folder/plot_functions.R" ,local = T)
# source("functions folder/table_functions.R" ,local = T)
# source("functions folder/text_functions.R" ,local = T)
# source("functions folder/produce_CA_content.R" ,local = T)
...
knit_result = parLapply(cl, Area, function(area){
  ...
  if(file.exists(paste0("temp/",area,"-content.rds"))){
    ...
    # sometimes the knitting fails but it's not reproducible [pandoc error 1]
    # this trycatch will attempt the knit and fail gracefully in parallel,
    # allowing code to continue.
    res = tryCatch(
      expr = {
        x = rmarkdown::render("CA_profile.Rmd",
                                output_dir = "output",
      ...
      error = function(e){ 
        # if there's an error knitting return -2
        return(-2)
      }
    )
